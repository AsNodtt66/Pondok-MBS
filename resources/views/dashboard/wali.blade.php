...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <!-- Card Informasi Wali dan Santri -->
    <div class="bg-gradient-to-br from-blue-800 to-indigo-500 rounded-2xl shadow-2xl p-6 transform transition duration-300 hover:scale-105 hover:shadow-3xl border border-indigo-300">
        <div class="flex items-center space-x-5">
            <div class="bg-white p-1 rounded-full shadow-md overflow-hidden border border-gray-200">
                <img src="{{ $wali->foto_profil ? asset('storage/' . $wali->foto_profil) : asset('images/default-profile.png') }}" alt="Foto Profil Wali" class="w-20 h-20 object-cover rounded-full">
            </div>
            <div class="flex-1">
                <h3 class="text-xl font-extrabold text-white drop-shadow-md">{{ $wali->Nama_wali }}</h3>
                <p class="text-yellow-200 font-semibold">Wali dari: {{ $santri->Nama_santri }}</p>
        ...
        <div class="mt-5 grid grid-cols-2 gap-3 text-white">
            <div class="bg-indigo-600/30 p-2 rounded-lg">
                <p class="text-xs text-yellow-100 uppercase tracking-wide">Hubungan</p>
                <p class="font-medium text-lg">{{ $wali->Hubungan ?? '-' }}</p>
            </div>
            <div class="bg-indigo-600/30 p-2 rounded-lg">
                <p class="text-xs text-yellow-100 uppercase tracking-wide">Kontak</p>
                <p class="font-medium text-lg">{{ $wali->Kontak ?? 'Tidak ada' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-br from-blue-700 to-slate-500 rounded-xl shadow-lg p-6 transform transition duration-300 hover:scale-105">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-white">Progress Hafalan {{ $santri->Nama_santri }}</h3>
        ...
        <div class="space-y-3 text-white">
            ...
            <div>
                ...
                <span class="px-2 py-1 text-xs rounded-full 
                    {{ optional($latestHafalan)->Kualitas_hafalan == 'Lancar' ? 'bg-green-100 text-green-600' : 
    ...
    <div class="bg-gradient-to-br from-blue-700 to-blue-400 rounded-xl shadow-lg p-6 transform transition duration-300 hover:scale-105">
        ...
        <div class="space-y-3 text-white">
            <div>
                <p class="text-sm text-yellow-200">Pembayaran Terakhir</p>
            ...
            <div class="flex justify-between">
                ...
                <span class="px-2 py-1 text-xs rounded-full 
                    {{ optional($latestPembayaran)->Status_bayar == 'lunas' ? 'bg-green-100 text-green-600' : 'bg-red-400 text-red-800' }}">
    ...
    <div class="relative z-10">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-white tracking-tight flex items-center">
                ...
                <span>Statistik Hafalan {{ $santri->Nama_santri }} (6 Bulan Terakhir)</span>
            </h3>
        ...
        <div class="text-center py-10 bg-indigo-800/20 rounded-lg backdrop-blur-sm">
            ...
            <p class="text-blue-100 text-sm max-w-md mx-auto mb-4">Santri Anda belum memiliki catatan hafalan untuk 6 bulan terakhir.</p>
